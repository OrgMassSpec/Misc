...
<https://r4ds.had.co.nz/model-basics.html> <https://www.jstor.org/stable/2346786>

`y ~ model` is interpreted as the response `y` modelled by the linear predictor `model`. A model consists of terms separated by `+` operators. The terms consist of variable and factor names separated by `:` operators. A term is interpreted as the interaction of all the variables and factors appearing in the term. The `*` operator denotes factor crossing. Factors are predictor variables, and can be crossed or nested.

See `model.matrix` for how the predictors are set up by the model formula.
